...
#define graph_index_t ap_int<32>
...
void accprof_hls_top (
    // INPUT PORTS
    ap_uint<32> nodeDataLs, ap_uint<32> nodeDataMs,	ap_uint<32> cpuCyclesLs,
    ap_uint<32> cpuCyclesMs, ap_uint<32> event0Ls, ap_uint<32> event0Ms,
    ap_uint<32> event1Ls, ap_uint<32> event1Ms, ap_uint<32> event2Ls,
    ap_uint<32> event2Ms, 	ap_uint<32> event3Ls, ap_uint<32> event3Ms,
    ap_uint<32> event4Ls, ap_uint<32> event4Ms, ap_uint<32> event5Ls,
    ap_uint<32> event5Ms,
    //OUTPUT PORTS
	ap_uint<8>* bram
)
...
#pragma HLS INTERFACE s_axilite port=return bundle=BUS_A
	...
	graph_index_t* termin_nibble = (graph_index_t*) (bram + TERMIN_OFFSET);
	...
	if((nodeDataLs & FUNC_ID_MASK) == FUNC_ID_MASK)
	{
		termin_nibble[0] = 0x1;
	}
	else if(nodeDataLs != 0)
	{
		if (sp > 0)
		{
