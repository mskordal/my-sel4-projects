...
/** Uncomment to inject calls to the software implementation of HLS */
#define USE_SW
/**@}*/
...
#ifndef USE_HLS
/*
 *Externally declared functions called from attester_sw library.
 */
Function *attesterTopFunc;	///< void attester_top_func(<16 uint32 params>)
							///< replaces the hardware design top function
Function *attesterPrintFunc;	///< void attester_print(void): prints call 							 	///< graph generated by attester_top_func
#endif
...
PreservedAnalyses InjectBRAMMod::run(Module &M, ModuleAnalysisManager &AM)
{
	...
	FunctionType * atfFt = FunctionType::get(Type::getVoidTy(context),
		atfParams, false);
	attesterTopFunc = Function::Create(atfFt, 
		Function::ExternalLinkage, "attester_top_func", &M);


	// Create an external declaration of sel4bench_init
	attesterPrintFunc = Function::Create(
		FunctionType::get(Type::getVoidTy(context), false), 
		Function::ExternalLinkage, "attester_print", &M);
#endif
	...
	for (Instruction *instr : retInsts)
	{
		...
		endBuilder.CreateCall(Mp->getFunction("attester_print"));
	...
	LLVMContext &context, int id)
{
...
#ifdef DEBUG_PRINT
	insertPrint(builder, "writeFuncStartToHlsBlock: ",
		ConstantInt::get(context, llvm::APInt(32, id)), bblk->getModule());
#endif
	...
	builder.CreateCall(attesterTopFunc, atfArgs);
#endif
	...
	LLVMContext &context)
{
	...
	// MSKORDALif we are in main we do not need to add to locals or restart events. Exit
