...
{
	byte_t* res;
	byte_t bkey[BYTE_KEY_SIZE];
	/*byte_t bkey[] = {0xb1, 0x6f, 0xfd, 0x1a, 0xf8, 0x37, 0x04};*/
	int bkey_len;
	sha256_ctx_t ctx;
	int i;

	bkey_len = ((key->subkey_idx*UINT32_WIDTH) + key->used_subkey_bits)/8;
	if(key->used_subkey_bits%8 != 0) bkey_len++;
	res = malloc(32*sizeof(byte_t));
	memcpy(bkey, key->key, bkey_len);
	memcpy(bkey + bkey_len, &nonce, sizeof(nonce));
	bkey_len += sizeof(nonce);
	/*for(i=0; i<7; i++)*/
	/*{*/
		/*printf("%x ", bkey[i]);*/
	/*}*/
	/*printf("\n");*/
	sha256_init(&ctx);
	sha256_update(&ctx, bkey, bkey_len);
