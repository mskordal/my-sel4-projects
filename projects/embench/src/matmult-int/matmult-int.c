...
#define RANDOM_VALUE (RandomIntegermm ())
#define ZERO 0
...
int Seedmm;
matrix ArrayAmm_ref, ArrayAmm, ArrayBmm_ref, ArrayBmm, ResultArray;

void Multiply (matrix A, matrix B, matrix Res);
void InitSeedmm (void);
void Test (matrix A, matrix B, matrix Res);
void Initialize (matrix Array);
int RandomIntegermm (void);

static int matmult_int_body (int  rpt);

void
matmult_int_warm (int  heat)
{
  int  res = matmult_int_body (heat);
...
int
matmult_int_run (void)
{
  return matmult_int_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
matmult_int_body (int rpt)
{
  int i;

  for (i = 0; i < rpt; i++)
    {
      memcpy (ArrayAmm, ArrayAmm_ref,
	      UPPERLIMIT * UPPERLIMIT * sizeof (ArrayAmm[0][0]));
      memcpy (ArrayBmm, ArrayBmm_ref,
	      UPPERLIMIT * UPPERLIMIT * sizeof (ArrayAmm[0][0]));

      Test (ArrayAmm, ArrayBmm, ResultArray);
    }
...
/*
 * Initializes the Seedmm used in the random number generator.
 */
void
InitSeedmm (void)
{
 ...
 */
int
RandomIntegermm (void)
{
  Seedmm = ((Seedmm * 133) + 81) % MOD_SIZE;
  return (Seedmm);
}
...
matmult_int_init ()
{
  ...
  for (OuterIndex = 0; OuterIndex < UPPERLIMIT; OuterIndex++)
    for (InnerIndex = 0; InnerIndex < UPPERLIMIT; InnerIndex++)
      ArrayAmm_ref[OuterIndex][InnerIndex] = RANDOM_VALUE;
  for (OuterIndex = 0; OuterIndex < UPPERLIMIT; OuterIndex++)
    for (InnerIndex = 0; InnerIndex < UPPERLIMIT; InnerIndex++)
      ArrayBmm_ref[OuterIndex][InnerIndex] = RANDOM_VALUE;
}

int
matmult_int_verify (int unused)
{
  int i, j;
  matrix exp = {
