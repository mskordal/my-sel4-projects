...
int
libminver_verify (int res __attribute ((unused)))
{
  int i, j;
  float eps = 1.0e-6;
...
static int libminver_body (int  rpt);

void
libminver_warm (int  heat)
{
  int  res = libminver_body (heat);
...
libminver_run (void)
{
  return libminver_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
libminver_body (int rpt)
{
  int i;
