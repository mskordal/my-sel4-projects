...
int
libhuffbench_verify (int res __attribute ((unused)))
{
...
void
libhuffbench_init (void)
{
}


static int libhuffbench_body (int rpt);

void
libhuffbench_warm (int heat)
{
  int res = libhuffbench_body (heat);
...
int
libhuffbench_run (void)
{
  return libhuffbench_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline)) libhuffbench_body (int rpt)
{
  int j;
