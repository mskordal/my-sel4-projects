...
#if defined(PRIME_N_PROBE) || defined(FLUSH_N_RELOAD)
...
#ifdef FLUSH_N_RELOAD
seL4_CPtr ep;
void attacker_func(void* arg1, void* arg2, void* arg3);
#endif
...
/*#ifdef FLUSH_N_RELOAD*/
	...
	/*sel4utils_thread_t attacker_thread;*/
	/*error = sel4utils_configure_thread(*/
		/*&vka, &vspace, &vspace, seL4_CapNull, seL4_CapInitThreadCNode, 0,*/
		/*&attacker_thread*/
	/*);*/
	/*ZF_LOGF_IFERR(error, "Failed to configure new thread");*/
	/*// Set affinity*/
	/*error = seL4_TCB_SetAffinity(seL4_CapInitThreadTCB, 0);*/
	/*ZF_LOGF_IFERR(error, "Failed to set affinity for main thread.");*/
	/*// Start the thread*/
	/*vka_object_t ep_object;*/
	/*vka_alloc_endpoint(&vka, &ep_object);*/
	/*ep = ep_object.cptr;*/
	/*ZF_LOGF_IFERR(error, "Failed to allocate endpoint.");*/
	/*sel4utils_start_thread(&attacker_thread, attacker_func, &ep, NULL, 1);*/
	/*error = seL4_TCB_SetAffinity(attacker_thread.tcb.cptr, 1);*/
	/*ZF_LOGF_IFERR(error, "Failed to set affinity for attacker thread.");*/
/*#endif*/
...
void attacker_func(void* arg1, void* arg2, void* arg3) {
	...
	int j;
	seL4_CPtr atk_ep = *((seL4_CPtr*)arg1);

	while(1) {
		seL4_Wait(atk_ep, NULL);
		printf("Cannot print that if not called by main\n");
		for (j=0; j<STREAM_ARRAY_SIZE; j+=DOUBLES_IN_LINE) {
			clflush(&a[j]);
			clflush(&b[j]);
			clflush(&c[j]);
		}
		...
		seL4_Send(atk_ep, seL4_MessageInfo_new(0, 0, 0, 0));
	}
}
#endif
